...
point_list++; x2=point_list->x; y2=point_list->y;
  ...
  if(y1==y2)       // horizontal line - either at top or bottom
      { 
        if(x2<xmin)
          {
          xmin=x2; xminl=(point_list->u)/2;
          }
        if(x2>xmax)
          {
          xmax=x2; xmaxl=(point_list->u)/2;
          }
    ...
    tex_draw(starttemp,x1,y1,x2,y2,((point_list-1)->u)/2,0,(point_list->u)/2,0);
    }
  else           // y1>y2  - due to anticlockwise, must be right side (going up - y getting smaller)
    { 
    endtemp=tex_endspace;
    tex_draw(endtemp,x2,y2,x1,y1,(point_list->u)/2,0,((point_list-1)->u)/2,0);
    }
...
point_list-=2; x2=point_list->x; y2=point_list->y;
  ...
  if(y1==y2)       // horizontal line
    ...
    {
    starttemp=tex_startspace;
    tex_draw(starttemp,x1,y1,x2,y2,((point_list+2)->u)/2,0,(point_list->u)/2,0);
    }
  else           // y1>y2  - due to anticlockwise, must be right side (going up - y getting smaller)
    { 
    endtemp=tex_endspace;
    tex_draw(endtemp,x2,y2,x1,y1,(point_list->u)/2,0,((point_list+2)->u)/2,0);
    }
