...
void distort_clip_edge(int num_lines, int edge, dcoord *input_list, dcoord *outlist, int *outlines);
...
//
// if you want to know what the current clipping area is call this function
//
void get_poly_clipping(Rect *clip_rect)
{
	clip_rect->top = screen_y_min;
	clip_rect->left = screen_x_min;
	clip_rect->bottom = screen_y_max;
	clip_rect->right = screen_x_max;
}
...
if(num_lines<2) 
  { 
  #if INPUT_CHECKING
  DebugStr("Insufficent lines - cannot clip"); 
  #endif
  }
...
switch(edge)
  ...
  default: report_error_c("Illegal case in visible","",0); return 1;
  }
...
if(edge<3)  // x axis clipping
  {
  if(x2==x1) { report_error_c("Cannot clip vertical line!","",0); }
...
//    } 
  ...
  x1=cut_point;
  y1+= static_cast<int>((y2-y1)*fraction_upline);
  u1+= static_cast<int>((u2-u1)*fraction_upline);
  v1+= static_cast<int>((v2-v1)*fraction_upline);
  }
else  // y axis clipping
  {
  if(y2==y1) { report_error_c("Cannot clip horizontal line!","",0); }

  fraction_upline=( (float) (cut_point-y1) / (float) (y2-y1) );

  x1+= static_cast<int>((x2-x1)*fraction_upline);
  y1=cut_point;
  u1+= static_cast<int>((u2-u1)*fraction_upline);
  v1+= static_cast<int>((v2-v1)*fraction_upline);
  }
