...
static pascal Boolean MyFilter(DialogPtr dlog, EventRecord *evt, short *itemHit)
	{
		...
		if (doHilite) {
			...
			if (type == (btnCtrl+ctrlItem)) {
				long soon = LS_TickCount60() + 7;		/* Or whatever feels right */
				HiliteControl((ControlHandle)hndl,1);
				while (LS_TickCount60() < soon) ;		/* Leave hilited for a bit */
				}
			}
		return(ans);
	}
...
static long strlen(const register char *str)
	{
		register char *p;
